...
import { AFFILIATE_REL, getMarketplaceLinks, trackAffiliateClick } from "@/lib/affiliate";
...
/* ──────────────────────────────────────────────────────────────────
 *  MarketplacePanel
 *  Lists per-activity deep links + Switzerland-scoped search links
 *  for every active partner. Direct activity links are sorted first
 *  and labelled "Open on X" instead of "Search on X" so users know
 *  they're going straight to the booking page.
 * ────────────────────────────────────────────────────────────────── */
function MarketplacePanel({ activity }: { activity: Activity }) {
  const links = getMarketplaceLinks(activity, "activity-detail-provider");
  if (links.length === 0) return null;

  const BRAND_COLORS: Record<string, string> = {
    swissactivities: "bg-rose-600 text-white",
    ...
    klook:           "bg-indigo-500 text-white",
    booking:         "bg-blue-700 text-white",
    omio:            "bg-pink-500 text-white",
  };
  ...
  return (
    <div className="mt-8 rounded-2xl border border-gray-100 bg-gray-50 p-6">
      <div className="flex flex-wrap items-start justify-between gap-2 mb-4">
        <div>
          <h3 className="font-semibold text-gray-900 text-base">
            {noPriced ? "Book this activity" : "Also check across platforms"}
          </h3>
          <p className="mt-0.5 text-xs text-gray-500">
            {noPriced
              ? "We don't have direct pricing yet — these marketplaces sell this activity. Prices and availability may differ."
              : <>Compare prices for <span className="font-medium text-gray-700">{activity.name}</span> across these marketplaces. You pay the same; we earn a small commission.</>}
          </p>
        </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        {links.map(({ partner, url, label, isDirectLink }) => (
          <a
            key={partner.id}
            href={url}
            target="_blank"
...
export function ActivityDetail({ activity }: { activity: Activity }) {
  ...
  const priceLabel = formatActivityPrice(activity, ageGroup);
      ...
      <div className="grid gap-8 lg:grid-cols-3">
        <div className="lg:col-span-2">
          <div className="relative aspect-[16/9] overflow-hidden rounded-2xl bg-gray-200 mb-4">
            <ActivityPhoto activity={activity} aspect="16/9" priority showCredit />
            <div className="pointer-events-none absolute inset-0 z-[1] bg-gradient-to-t from-slate-950/45 to-transparent" />
            ...
            <div className="absolute bottom-4 left-4 right-4 z-10 flex flex-wrap items-end justify-between gap-3">
              ...
              <div className="rounded-2xl bg-white/92 px-4 py-3 text-right shadow-lg backdrop-blur">
                <p className="text-xs font-medium uppercase tracking-wide text-gray-400">
                  {hasPriced ? "Best price" : "Pricing"}
                </p>
                <p className="mt-1 text-2xl font-bold text-gray-900">
                  {bestPrice === null ? "Check below" : bestPrice === 0 ? "Free" : `CHF ${bestPrice}`}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="mt-8 rounded-[28px] border bg-white p-6 shadow-sm">
            ...
            {activeTab === "providers" && (
              <div className="mt-6">
                {hasPriced && recommendedProvider && cheapest && bestRated ? (
                  <>
                  ...
                  <div className="rounded-2xl border border-dashed border-gray-200 bg-gray-50 p-6 text-sm text-gray-600">
                    <p className="font-medium text-gray-800">Pricing not yet on file</p>
                    <p className="mt-1 leading-6">
                      We&apos;re still collecting direct provider prices for {activity.name}. In the meantime, the booking marketplaces below all sell this activity — check current availability and pricing on the platform of your choice.
                    </p>
                  </div>
                )}

                {/* Marketplace comparison panel — also covers the "no priced provider" case */}
                  ...
                  <Card className="border-0 bg-gray-50 shadow-none">
                    ...
                    <CardContent className="space-y-3 text-sm text-gray-600">
                      {recommendedProvider ? (
                        ...
                        <div className="rounded-lg bg-white p-3">
                          No direct provider data on file yet — use the marketplace links below to compare.
                        </div>
                      )}
                      <div className="rounded-lg bg-white p-3">
            ...
            <CardContent className="space-y-4">
              {recommendedProvider ? (
                ...
                <div className="rounded-2xl border border-gray-100 bg-gray-50 p-4">
                  <p className="text-xs font-medium uppercase tracking-wide text-gray-500">Direct pricing</p>
                  <p className="mt-1 font-semibold text-gray-900">Coming soon</p>
                  <p className="mt-1 text-sm text-gray-600">Use the marketplace links below to check live availability and price.</p>
                </div>
              )}
              <AffiliateDisclosure />
          ...
          <div className="space-y-3">
            {cheapest ? (
              ...
              <Button
                ...
                size="lg"
                onClick={() => {
                  document.getElementById("marketplace-panel")?.scrollIntoView({ behavior: "smooth", block: "start" });
                }}
              >
